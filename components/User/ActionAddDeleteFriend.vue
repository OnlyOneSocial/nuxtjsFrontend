...
<template>
    <div :title="title" @click="action">
        <i v-if="friendStatus.status===3" class="icon icon-add-friend" style="--size:16px" title="Отправить заявку в друзья" />
...
export default {
    ...
    computed: {
        title () {
            if (this.friendStatus && this.friendStatus.status >= 0) {
                if (this.friendStatus.status === 3) {
                    return 'Отправить заявку в друзья'
                }
                if (this.friendStatus.status === 1) {
                    return 'Удалить из друзей'
                }
                if (this.friendStatus.status === 0 && this.friendStatus.forme === true) {
                    return 'Принять заявку в друзья'
                }
                if (this.friendStatus.status === 0 && this.friendStatus.forme === false) {
                    return 'Отменить заявку в друзья'
                }
            }
            return ''
        }
    ...
    methods: {
        ...
        async RequestAccept () {
            ...
            if (info.data.request.status) {
                this.$store.commit('UserPage/SetFriendStatus', {
                    status: 1,
                    forme: true
                })
            }
        ...
        async Request () {
            ...
            if (info.data.request.status) {
                this.$store.commit('UserPage/SetFriendStatus', {
                    status: info.data.request.status,
                    forme: false
                })
            }
        },
